...
class LowStockLevelProducts extends StatefulWidget {
  const LowStockLevelProducts({super.key});

  @override
  State<LowStockLevelProducts> createState() => _LowStockLevelProductsState();
}

class _LowStockLevelProductsState extends State<LowStockLevelProducts> {
  bool expanded = false;
  String productId = "";
    ...
    return  Scaffold(
      ...
      title: Row(children: [
        Expanded(child: Column(
          ...
          children: [
            heading2(text: "Products with low stock level"),
          ],
        ))
      ],) 
      ,),
                      ...
                      duration: Duration(milliseconds: 300),
                       ...
                       GetX<ProductController>(
                        init: ProductController(),
                         builder: (find2) {
                           return Column(children:find2.productsWithStock.where((element) => element.name.toLowerCase().contains(find.searchKeyword.value.toLowerCase())).where((product) => product.availableStock <= product.lowAmount ).map((product) => Padding(
